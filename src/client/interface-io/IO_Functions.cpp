...
namespace basic_http_client::IO_Functions {
    ...
    namespace {
        ...
        const int WAIT_FOR_DATA = -101;

        int send_(HttpClient *client, int sent_bytes) {
        ...
        int recv_(HttpClient *client, int total_recvd) {
        ...
        int send_async(HttpClient *client, int total_sent) {
            int res = 0;
            int sent = 0;
        ...
        int recv_async(HttpClient *client, int total_recvd) {
            int res = 0;
            int recvd = 0;
            res = poll(client->pollFd_, 1, 0);
